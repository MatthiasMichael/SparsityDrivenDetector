...
for dbFile in dbFiles:
    ...
    tableNames = [x[0] for x in c.execute('SELECT name FROM sqlite_master WHERE type=\'table\';')]

    timingTables = [x for x in tableNames if x.startswith('Timing_')]

    plt.clf()
    plt.cla()

    for table in timingTables:
        if not table.startswith('Timing_Controller'):
            continue

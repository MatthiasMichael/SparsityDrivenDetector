...
experimentFolders = [
    ...
    # r'\\datafs.ini.rub.de\rtcv\Experimente\SDD_SFS_Fusion (ITSC 2019)\out_experiment_003'
        r'C:\TEMP\SDD_Experiments\out_experiment_002'
        ]
outFolders = [
    ...
    r'C:\TEMP\SDD_Experiments\out_experiment_002\figures'
        ]
...
\toprule
\textbf{$d_{\text{grid}}$} & \textbf{$t_\text{merge}$} & \textbf{$t_\text{segm}$} & \textbf{$e_\text{mean}$} & \textbf{Prec} & \textbf{Rec} & \textbf{Time (ms)} \\ 
\midrule
"""
    ...
    for dbFile in dbFiles:
        ...
        configuration = [x for x in c.execute("SELECT `grid.distX`, `detector.maxMergingDistance`, `sfs.init.voxelSize`, `sfs.param.minPartSegmentation` FROM 'Configuration';")]
        if len(configuration) != 1:
            raise RuntimeError("Configuration should only have one row.")

        if configuration[0][2] != 30:
            continue

        expected_error_min = float(configuration[0][0]) * expectation_dist

        s_temp = '{:2.0f} & {:3.0f} & {:1.2f} &'.format(configuration[0][0], configuration[0][1], configuration[0][3])
        s += s_temp
        #s += str(configuration[0][0]) + ' & ' + str(configuration[0][2]) + ' & ' + str(configuration[0][1]) + ' & ' + str(configuration[0][3]) + ' & '
    ...
    s += "\end{tabular}"
    with open(join(outFolder, 'table_30.tex'), 'w') as outF:
        outF.write(s)

...
int main(int argc, char ** argv)
{
	...
	// PARAMETERS
	const std::string optimizationType = "single";

	const std::string sceneFile = "C:/TEMP/test002.scene";
	const std::string dictFile = "C:/TEMP/dict.zip";
	...
	const auto dictionary = Dictionary::load(dictFile);

	auto detector = SparsityDrivenDetector(dictionary, makeFactory_withDecorator<OptimizationProblemTimer>(optimizationType));

	auto input = std::make_unique<VideoInput_Mpeg>(sceneInfo, dictionary.getTargetImageSize(), dictionary.getTargetImageSize(),
		&preprocessing, startFrameNumber);

	// END SETUP

	MainWindow w;
	initVisuals("SDD_UEScene_LoadDict", &w);
